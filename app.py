...
class DiseaseProbabilityCalculator:
    def __init__(self, csv_data=None):
        ...
        Args:
            csv_data (DataFrame): DataFrame with columns MemberID, ICDCode, Age, GenderID
        """
    ...
    def get_pattern_age_gender_analysis(self, pattern_tuple):
        """Get age and gender analysis for a specific pattern"""
        try:
            if pattern_tuple not in self.disease_patterns:
                return {
                    'success': False,
                    'error': 'Pattern not found in current analysis'
                }
            
            patients_with_pattern = self.disease_patterns[pattern_tuple]
            
            # Get age and gender data for patients with this pattern
            pattern_patient_ids = [p['patient_id'] for p in patients_with_pattern]
            
            # Filter patient data for those with this pattern
            pattern_data = self.patient_data[self.patient_data['MemberID'].isin(pattern_patient_ids)]
            
            # Check if Age and GenderID columns exist
            if 'Age' not in pattern_data.columns or 'GenderID' not in pattern_data.columns:
                return {
                    'success': False,
                    'error': 'Age and/or GenderID columns not found in dataset'
                }
            
            # Get unique patients (since patients may have multiple ICD codes)
            unique_patients = pattern_data.drop_duplicates('MemberID')
            
            # Age bucket analysis
            unique_patients['AgeBucket'] = unique_patients['Age'].apply(self.get_age_bucket)
            age_distribution = unique_patients['AgeBucket'].value_counts().to_dict()
            
            # Gender analysis
            gender_distribution = unique_patients['GenderID'].value_counts().to_dict()
            
            # Age and gender combined analysis
            age_gender_crosstab = pd.crosstab(
                unique_patients['AgeBucket'], 
                unique_patients['GenderID']
            ).to_dict()
            
            # Statistical analysis
            total_patients = len(unique_patients)
            avg_age = float(unique_patients['Age'].mean())
            median_age = float(unique_patients['Age'].median())
            min_age = int(unique_patients['Age'].min())
            max_age = int(unique_patients['Age'].max())
            
            # Age bucket percentages
            age_percentages = {
                bucket: (count / total_patients * 100) 
                for bucket, count in age_distribution.items()
            }
            
            # Gender percentages
            gender_percentages = {
                gender: (count / total_patients * 100) 
            ...
            return {
                ...
                'age_statistics': {
                    'mean': round(avg_age, 1),
                    'median': median_age,
                    'min': min_age,
                    'max': max_age,
                    'range': max_age - min_age
                },
                'insights': self._generate_age_gender_insights(
                    age_distribution, gender_distribution, 
                    age_percentages, gender_percentages, avg_age
                )
            }
    ...
    def _generate_age_gender_insights(self, age_dist, gender_dist, age_pct, gender_pct, avg_age):
        """Generate insights based on age and gender analysis"""
        insights = []
        
        # Most common age bucket
        most_common_age_bucket = max(age_dist, key=age_dist.get)
        insights.append(f"Most common age group: {most_common_age_bucket} ({age_pct[most_common_age_bucket]:.1f}%)")
        
        # Gender distribution insight
        if len(gender_dist) > 1:
            dominant_gender = max(gender_dist, key=gender_dist.get)
            insights.append(f"Gender distribution: {dominant_gender} {gender_pct[dominant_gender]:.1f}%, Other {100-gender_pct[dominant_gender]:.1f}%")
        else:
            only_gender = list(gender_dist.keys())[0]
            insights.append(f"All patients are {only_gender}")
        
        # Age-related insights
        if avg_age < 30:
            insights.append("This pattern is predominantly seen in younger patients")
        elif avg_age > 65:
            insights.append("This pattern is predominantly seen in older patients")
        else:
            insights.append("This pattern affects middle-aged patients")
        
        # Risk group insights
        elderly_buckets = ['65-75', '75-85', '85-95', '95+']
        elderly_count = sum(age_dist.get(bucket, 0) for bucket in elderly_buckets)
        elderly_percentage = (elderly_count / sum(age_dist.values())) * 100
        
        if elderly_percentage > 70:
            insights.append("High prevalence in elderly population (65+)")
        elif elderly_percentage < 20:
            insights.append("Low prevalence in elderly population (65+)")
    ...
    def load_data(self):
        ...
        try:
            ...
            # Check for optional Age and GenderID columns
            has_age_gender = 'Age' in self.patient_data.columns and 'GenderID' in self.patient_data.columns
            ...
            return {
                ...
                'has_age_gender_data': has_age_gender
            }
        ...
        try:
            ...
            if result['success']:
                ...
                return render_template('analysis.html', 
                                     ...
                                     has_age_gender_data=result.get('has_age_gender_data', False))
            else:
...
def pattern_analysis():
    """New API endpoint for age and gender analysis of a specific pattern"""
    global calculator
    ...
    try:
        ...
        pattern_str = request.json.get('pattern', '').strip()
        
        if not pattern_str:
            return jsonify({'success': False, 'error': 'Pattern is required.'})
        
        # Convert pattern string back to tuple
        pattern_codes = [code.strip() for code in pattern_str.split(' + ')]
        pattern_tuple = tuple(sorted(pattern_codes))
        
        # Get age and gender analysis for this pattern
        analysis_result = calculator.get_pattern_age_gender_analysis(pattern_tuple)
